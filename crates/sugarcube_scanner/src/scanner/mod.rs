...
static SHOULD_TRACE: sync::LazyLock<bool> = sync::LazyLock::new(
    || matches!(std::env::var("DEBUG"), Ok(value) if value.eq("*") || (value.contains("sugarcube:scanner") && !value.contains("-sugarcube:scanner"))),
);
...
fn init_tracing() {
    ...
    let file_path = format!("sugarcube-{}.log", std::process::id());
...
fn create_walker(sources: Sources) -> Option<WalkBuilder> {
    ...
    for ignore in auto_source_detection::RULES.iter() {
        // Skip adding ignore rules for now - API changed
    }
