...
impl From<ChangedContent> for sugarcube_scanner::ChangedContent {
  fn from(changed_content: ChangedContent) -> Self {
    if let Some(file) = changed_content.file {
      return sugarcube_scanner::ChangedContent::File(file.into(), changed_content.extension);
    }

    if let Some(contents) = changed_content.content {
      return sugarcube_scanner::ChangedContent::Content(contents, changed_content.extension);
    }
...
impl From<GlobEntry> for sugarcube_scanner::GlobEntry {
...
impl From<sugarcube_scanner::GlobEntry> for GlobEntry {
  fn from(glob: sugarcube_scanner::GlobEntry) -> Self {
    Self {
      base: glob.base,
      pattern: glob.pattern,
    }
...
impl From<SourceEntry> for sugarcube_scanner::PublicSourceEntry {
...
#[napi]
pub struct Scanner {
  scanner: sugarcube_scanner::Scanner,
}
...
impl Scanner {
  ...
  pub fn new(opts: ScannerOptions) -> Self {
    Self {
      scanner: sugarcube_scanner::Scanner::new(match opts.sources {
